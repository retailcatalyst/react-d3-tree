...
class Tree extends React.Component {
    constructor() {
        ...
        this.handleRemoveNode = (nodeId, parentNodeId, callback) => {
            const data = clone(this.state.data);
        ...
        this.handleUpdateNodeAttributes = (nodeId, node, callback) => {
            const data = clone(this.state.data);
            const matches = this.findNodesById(nodeId, data, []);
            if (matches.length > 0) {
                const targetNodeDatum = matches[0];
        ...
        this.handleAddChildrenToNode = (nodeId, childrenData, replace, callback) => {
            const data = clone(this.state.data);
            const matches = this.findNodesById(nodeId, data, []);
            if (matches.length > 0) {
                const targetNodeDatum = matches[0];
    ...
    componentDidUpdate(prevProps) {
        if (this.props.data !== prevProps.data) {
            if (this.state.isInitialRenderForDataset) {
                ...
                this.setState({
                    data: Tree.assignInternalProperties(clone(this.props.data), undefined, this.state.data[0]),
                });
            }
        }
     ...
     * @static
     */
    static assignInternalProperties(data, currentDepth = 0, prevDataState) {
        ...
        return d.map(n => {
            ...
            nodeDatum.__rd3t = (_b = (_a = prevNodeDatum === null || prevNodeDatum === void 0 ? void 0 : prevNodeDatum.__rd3t) !== null && _a !== void 0 ? _a : nodeDatum.__rd3t) !== null && _b !== void 0 ? _b : { id: null, depth: null, collapsed: false };
            ...
            if (nodeDatum.children && nodeDatum.children.length > 0) {
                nodeDatum.children = Tree.assignInternalProperties(nodeDatum.children, currentDepth + 1, prevDataState);
            }
            return nodeDatum;
        });
    }
            ...
            React.createElement("svg", { className: `rd3t-svg ${this.svgInstanceRef} ${svgClassName}`, width: "100%", height: "100%" },
                React.createElement(TransitionGroupWrapper, { enableLegacyTransitions: enableLegacyTransitions, component: "g", className: `rd3t-g ${this.gInstanceRef}`, transform: `translate(${translate.x},${translate.y}) scale(${scale})` },
                    ...
                    nodes.map((hierarchyPointNode, i) => {
                        ...
                        return (React.createElement(Node, { key: 'node-' + hierarchyPointNode.data.__rd3t.id, data: data, position: { x, y }, hierarchyPointNode: hierarchyPointNode, parent: parent, nodeClassName: this.getNodeClassName(parent, data), renderCustomNodeElement: renderCustomNodeElement, nodeSize: nodeSize, orientation: orientation, enableLegacyTransitions: enableLegacyTransitions, transitionDuration: transitionDuration, onNodeToggle: this.handleNodeToggle, onNodeClick: this.handleOnNodeClickCb, onNodeMouseOver: this.handleOnNodeMouseOverCb, onNodeMouseOut: this.handleOnNodeMouseOutCb, handleAddChildrenToNode: this.handleAddChildrenToNode, subscriptions: subscriptions, centerNode: this.centerNode, handleRemoveNode: this.handleRemoveNode, handleUpdateNodeAttributes: this.handleUpdateNodeAttributes }));
                    })))));
    }
}
Tree.defaultProps = {
...
Tree.getNodeProps = (node, nodeKey) => {
    if (node.key === nodeKey) {
        return node;
    }
    else {
